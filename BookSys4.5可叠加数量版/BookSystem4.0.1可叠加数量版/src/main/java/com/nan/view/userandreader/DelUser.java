...
public class DelUser extends JFrame {
    ...
    public void delUser(){
        ...
        jLaccount = new JLabel("用户ID");//实例化账号ID文本域，并为账号文本域赋值
        jLaccount.setFont(font2);//设置账号文本域的文字的大小格式
        jLaccount.setBounds(30,20,220,55);//账号ID文本域的位置大小设置
        jLName = new JLabel("用户为:");//实例化账号内容文本域,初始化内容为  账号为：
        ...
        bdel = new JButton("删除用户");//删除账户的按钮  实例化
        ...
        bdel.addMouseListener(new MouseListener() {//为删除按钮添加事件监听器
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                //下面这2行貌似用不着
                //delAccount = Gain.gain(textField);//将从删除账号的账号文本框获取的文本信息赋值到已经声明的删除账号字符串中
                //delPass = Gain.gain(textField2);//将从删除账号的密码文本框获取的文本信息赋值到已经声明的删除账号的密码字符串中


                if(new DelUserByIdDao().compare(account,password))  {  //管理员账号密码和数据库横纵的密码进行比对

                    new DelUserByIdDao().delUser();//根据静态变量ID进行删除
        ...
        bse.addMouseListener(new MouseListener() {//为重置按钮添加鼠标监听器
            ...
            public void mouseClicked(MouseEvent e) {//添加鼠标单击事件
                ...
                textField2.setText(StaticData.delUsername);//修改文本框内容，内容为要修改的书籍名称

                if(StaticData.delUsername != ""){  //当静态字符串bokname2不为空，bokname2有内容时进行此判断
                    bdel.setEnabled(true);//删除"删除用户"按钮进入激活可按状态
                   jLaccount2.setEnabled(true);//管理员账号文本域设置为可用
                    jLpass2.setEnabled(true);//管理员密码文本域设置可用
        ...
        addWindowListener(new WindowAdapter() {//添加窗体监听器
            ...
            public void windowClosing(WindowEvent e) {//窗口关闭时执行的代码
                ...
                Only.delUserCount = 0;  //countup重置为0
            }
