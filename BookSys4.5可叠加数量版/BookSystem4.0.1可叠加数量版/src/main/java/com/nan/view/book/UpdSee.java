...
/**
 * 修改图书内容的类,用于查询修改图书的id,然后根据ID得到图书信息，并打开修改图书信息的界面
 */
public class UpdSee extends JFrame {
    ...
    public void upd(){ //修改图书内容的主窗体
        Only.UpdSeeCount = 1;//设置窗口状态为已存在,1为存在，0为不存在
        ...
        //jPanel3.setBounds(0,155,300,108);//设置面板3的大小以及位置 155  108//原来的
        jPanel3.setBounds(23,155,250,128);//设置面板3的大小以及位置 155  108
        jPanel3.setLayout(null);//面板3单独使用绝对布局，为了按钮美观
        jPanel4.setBounds(0,288,300,88);//设置面板4的大小以及位置 275  88
        ...
        bupdata.setBounds(0,420,180,65);// 420
        bup = new JButton("查询书名");
        bup.setFont(font2);//按钮文字采用28号
//        bup.setBounds(0,310,180,65);//原来的
        bup.setBounds(53,70,150,48);
        ...
        textField2.setText("自动显示修改的书");//设置目标修改图书文本框的默认显示内容
        ...
        //jPanel3.setBorder(BorderFactory.createLineBorder(Color.BLUE, 5));//面板添加边框
        ...
        bupdata.addMouseListener(new MouseListener() {//为修改信息按钮添加鼠标监听器
            ...
            public void mouseClicked(MouseEvent e) {//鼠标单击事件
                ...
                if(Only.UpdataCount == 0){//当该窗口不存在时可以创建该窗口
        ...
        bup.addMouseListener(new MouseListener() {//为查询按钮添加鼠标监听器
            ...
            public void mouseClicked(MouseEvent e) {//鼠标点击事件
                ...
                textField2.setText(StaticData.bokName);//修改文本框内容，内容为要修改的书籍名称
                if(StaticData.bokName != ""){  //当静态字符串bokname不为空，bokname有内容时进行此判断
...
//                MainApp.staticdata.countup = 0;  //countup重置为0
                Only.UpdSeeCount = 0;//设置窗口不存在，可创建
            }
