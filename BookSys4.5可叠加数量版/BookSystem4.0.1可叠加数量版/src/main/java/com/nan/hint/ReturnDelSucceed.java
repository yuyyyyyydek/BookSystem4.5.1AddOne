 ...
 * 归还图书成功的消息弹窗
 */
public class ReturnDelSucceed extends DelWin{
    Font font2 = new Font("Serif", Font.BOLD, 28);//设置文字大小
    public void win(){
        //MainApp.staticdata.countrewin = 1;//本窗体是否已经存在以及只能存在一个该窗体的判断代码
        JButton win;//声明一个按钮
        JLabel jLabel ;//登录成功的提示
        JFrame jwin = new JFrame();//创建一个窗体对象
        jwin.setTitle("借阅图书归还成功");
        ...
        jLabel = new JLabel("借阅图书归还成功");//实例化文本域
        ...
        win.addMouseListener(new MouseListener() {//定义修改成功窗口中按钮的监听事件
            ...
            public void mouseClicked(MouseEvent e) {//鼠标单击事件
                ...
                //MainApp.staticdata.countrewin = 0;//将验证是否存在此窗口的判断重置为0，方便下次再次弹出成功信息窗口
            }
        ...
        jwin.addWindowListener(new WindowAdapter() {//添加窗体监听器
            ...
            public void windowClosing(WindowEvent e) {//窗口关闭时执行的代码
                ...
                //MainApp.staticdata.countrewin = 0;  //countup重置为0
            }
