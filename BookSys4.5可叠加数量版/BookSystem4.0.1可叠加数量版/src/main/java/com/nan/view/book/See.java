...
//    private JPanel jpanel3;//面板3，用来接收其他类传过来的
    public SeeMid seeMid = new SeeMid();
    private JTable jtable2;//声明表格
...
//    private JPanel jPanel2;//声明面板2
    ...
    public void see2() throws SQLException, ClassNotFoundException {

//        jPanel2 = new JPanel();//创建面板2
        Only.SeeCount = 1;//设置该窗口是否存在，这里赋值为0，表示该窗口已经存在
...
//        setVisible(true);//设置窗体可见
        ...
        getContentPane().add(new JScrollPane(jtable)); // 使用滚动面板以防表格太大****
        ...
        bse.addMouseListener(new MouseListener() {//为查询按钮添加鼠标监听器
            ...
            public void mouseClicked(MouseEvent e) {//鼠标点击事件
                //jPanel2.removeAll();//移除面板2中所有的内容//这里是原始行
//                jpanel3.setVisible(false);//移除面板2中所有的内容
...
////                    jpanel3.removeAll();
////                    jpanel3 = seeMid.see(booktext);//查询带有"booktext"字段的图书信息
////
////                    seeBook(booktext);
...
//        setVisible(true);//设置窗口内容可见
        ...
        this.addWindowListener(new WindowAdapter() {//窗口关闭执行的代码
            ...
            public void windowClosing(WindowEvent e) {//关闭窗口时执行的事件
                ...
                Only.SeeCount = 0;//设置该窗口是否存在，这里赋值为0，表示不存在，方便下次创建
...
//        String[] columnNames = {"ID", "BookName", "BookAuthor","Price","Amount","BookType"};//定义表格列名
        ...
        //创建表格的数据模型
        //DefaultTableModel model = new DefaultTableModel(columnNames,0); //这行是原始的
        //DefaultTableModel model = seeMid.seeall();

        //jtable = new JTable(Only.model);//表格模型添加到表 //这行是原始的


 //---
//        SeeMid seeMid = new SeeMid();
//        jpanel3 = seeMid.seeAll();
//        jpanel3.setLocation(300,0);
...
//        setVisible(true);
  ...
  //---
               ...
               // DefaultTableModel model = seeMid.seeAll();//表格模型添加到表

        //JTable table = new JTable(model);

        // 将 JTable 添加到 JScrollPane 中（可选，但通常用于改善显示效果）
        //JScrollPane scrollPane = new JScrollPane(table);

        // 将 JScrollPane（包含 JTable）添加到 JFrame 的内容面板中
        //getContentPane().add(scrollPane);

        // 设置 JFrame 可见

//        repaint();//刷新重绘
//        jPanel2.add(new JScrollPane(jtable)); // 使用滚动面板以防表格太大****//这行是原始的

        //此处创建SeeMid中间层类的实例
//        SeeMid seeMid = new SeeMid();
//        getContentPane().add(seeMid.seeAll());
//        jPanel2.add(seeMid.seeAll());

//        jPanel2.add(new JScrollPane(jtable));//这行是原始的

    }

    public void seeAllBooks() throws SQLException, ClassNotFoundException {
        //原先seeThis，主要用于更改model
        DefaultTableModel model = seeMid.seeAll();
...
//        setVisible(true);
    ...
    public void seeBook(String booktext) throws ClassNotFoundException {
        //主要用于更改model
        DefaultTableModel model = seeMid.see(booktext);
        this.jtable.setModel(model);

//        jpanel3 = seeMid.see(booktext);
//        jpanel3.setLocation(300,0);
//        //getContentPane().add(jpanel3);
//        add(jpanel3);
//        setVisible(true);
