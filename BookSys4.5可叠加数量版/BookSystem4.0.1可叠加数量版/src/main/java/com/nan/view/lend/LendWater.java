...
import com.nan.middle.LendWaterMid;
...
public class LendWater extends JFrame {
    ...
    LendWaterMid lendWaterMid = new LendWaterMid();
    ...
    public void lendWater() {
//        jPanel2 = new JPanel();//面板2实例化


        Only.lendWaterCount = 1;//设置借阅流水窗体只能存在一个，1表示已经创建，0表示未创建，为1 后不可再次创建
...
//        setVisible(true);//设置窗口内容可见
        ...
        bseename.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (lendername != null) {//当借阅者姓名文本框中的内容不为空的时候，执行此if中的内容
                    ...
                    lendWaterMid.seeLender(lendername);//查询带有lendername中内容的借阅信息
                    repaint();//刷新重绘
                    setVisible(true);//设置窗口内容可见
        ...
        bseemobile.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                lendWaterMid.seeMobile(lendermobile);//查询带有lendername中内容的借阅信息
                repaint();//刷新重绘
                setVisible(true);//设置窗口内容可见
        ...
        addWindowListener(new WindowAdapter() {//添加窗体监听器
            ...
            public void windowClosing(WindowEvent e) {//窗口关闭时执行的代码
                ...
                Only.lendWaterCount = 0;  //countlendwater重置为0
            }
...
//    }
    ...
    public void seeAllLender(){
        jPanel3.removeAll();//移除面板3所有内容
        jPanel3 = lendWaterMid.seeAllLender();;//显示所有图书信息
