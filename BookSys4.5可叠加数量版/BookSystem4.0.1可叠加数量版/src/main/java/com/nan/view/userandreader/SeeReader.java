...
import com.nan.middle.SeeReaderMid;
 ...
 * 读者查询功能，查询读者的窗口
 */
public class SeeReader extends JFrame{
    ...
    SeeReaderMid seeReaderMid = new SeeReaderMid();//此处切换类声明，记得改SeeReaderMid
        ...
        public void seeReader(){
            Only.seeReaderCount = 1;//设置该查询窗口已经存在,1表示存在，0表示不存在

            setSize(890,610);//查询窗口的大小
            setTitle("读者查询");//设置窗口标题
            setLayout(null);//设置窗体布局为绝对布局
            setLocationRelativeTo(null);//c窗口居中显示
            setResizable(false);//设置窗体是否可以改变大小

            jLabel = new JLabel("输入读者账号可查询");//查询提示文字
            bse = new JButton("读者查询");//查询按钮
            ...
            jPanel = seeReaderMid.seeAllReaderUser();//显示所有信息，调用类方法获得面板内容
            add(jPanel);//添加面板到本窗体
            ...
            bse.addMouseListener(new MouseListener(){
                ...
                public void mouseClicked(MouseEvent e) {
                    ...
                    if(userText == null ||userText.equals("")){//如果文本框中为空
                        ...
                        jPanel = seeReaderMid.seeAllReaderUser();//显示所有信息
                        add(jPanel);//添加面板到窗体
                        jPanel.setVisible(true);//面板设置可见
                    ...
                    if(userText!=null){//当文本框中的内容不为空的时候，执行此if中的内容
                        ...
                        jPanel = seeReaderMid.seeReaderUserByName(userText);//根据userText内容查询内容
                        add(jPanel);//面板添加到窗体
            ...
            addWindowListener(new WindowAdapter() {//添加窗体监听器
                ...
                public void windowClosing(WindowEvent e) {//窗口关闭时执行的代码
                    ...
                    Only.seeReaderCount = 0;  //countup重置为0
                }
